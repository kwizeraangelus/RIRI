...
const [profileForm, setProfileForm] = useState({
  ...
  location: '',
  university: '',
  details: '' // This maps to "Bio" or "Short Description"
});
const [imagePreview, setImagePreview] = useState(null);
 ...
 const fetchUserAndInnovations = useCallback(async () => {
    ...
    try {
      const [userRes, innovationsRes] = await Promise.all([
        ...
        fetch(getApiUrl('/api/my-innovations/'), {
          headers: { 'Authorization': `Bearer ${token}` },
        })
      ]);
  ...
  // ✅ This is the key change - use a separate effect for initial load
  useEffect(() => {
    let isMounted = true;

    const loadData = async () => {
      if (isMounted) {
        await fetchUserAndInnovations();
      }
    };
    loadData();
 ...
 const saveProfile = async (e) => {
  ...
  // Append all fields from the profileForm
  ...
  try {
    ...
    if (res.ok) {
      ...
      setUser(updated); // Update the UI with new data
                ...
                {innovations.map(innovation => {
                  ...
                  return (
                    <div key={innovation.id} className="group bg-white border-2 border-purple-100 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition transform hover:scale-105 relative">
                      {innovation.photo ? (
                        <img src={getApiUrl(innovation.photo)} alt={innovation.name} className="w-full h-48 object-cover" />
                      ) : (
