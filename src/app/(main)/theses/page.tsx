...
export default function thesesPage() {
  ...
  const [inputValue, setInputValue] = useState(''); // For real-time display
  ...
  // Proper debounced search handler
  const handleSearchInputChange = useCallback((value: string) => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      setSearchTerm(value.trim()); // Update search after 400ms
    }, 400);

    // Show results if input has content
  ...
  // Fetch publications when search term or filters change
  useEffect(() => {
    fetchPublications();
  }, [fetchPublications]);

  // Fetch counts when degree filter changes
  useEffect(() => {
    fetchCounts();
  }, [degreeFilter]);
          ...
          <div className="relative flex justify-center mb-12 z-30">
            <div className="w-full max-w-2xl relative">
              <input
                ...
                onFocus={() => {
                  if (inputValue.trim().length > 1) setShowSearchResults(true);
                }}
          ...
          {!showSearchResults && (
            <>
              <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-8 mb-12">
                ...
                >
                  FYP <span className="text-lg font-normal opacity-90">({counts.dissertation})</span>
                </button>
              </div>
