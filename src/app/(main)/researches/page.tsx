  ...
  return (
    <div className="min-h-screen bg-[#E0F2FE] text-gray-900">
      ...
      <section className="py-16 px-6 -mt-10">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-[#050A14]">
            ...
            <div className="text-center py-24 bg-white rounded-2xl border border-gray-100 shadow-sm">
              {query.trim() ? (
                <>
                  ...
                  <p className="text-gray-400 italic text-sm">No results for &ldquo;{query}&rdquo;</p>
                  <button onClick={() => setQuery('')} className="mt-3 text-xs text-blue-600 hover:underline">
                    Clear search
                  </button>
                </>
              ) : (
                <p className="text-gray-400 italic">No approved publications found.</p>
              )}
            </div>
          ) : (
            <div className="flex flex-col gap-2">
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-6 py-4 gap-4 text-left hover:bg-gray-50 transition-colors"
                    >
                      <span className="font-medium text-[#050A14] text-sm flex-1">
                        {highlight(pub.title || 'Untitled', q)}
                      </span>
                      <span className="text-gray-400 text-xs italic flex-shrink-0 hidden sm:block">
                    ...
                    {/* EXPANDED DETAIL */}
                    {isOpen && (
                      <div className="px-6 pb-5 pt-2 bg-gray-50 border-t border-gray-100 text-sm text-gray-600 space-y-2">
                        {pub.publication_type && (
                          <span className="inline-block text-[11px] font-bold uppercase tracking-wider bg-blue-100 text-blue-700 px-2 py-0.5 rounded">
                            {pub.publication_type}
                          </span>
                        )}
                        ...
                        {pub.url && (
                          <a
                            ...
                            className="inline-flex items-center gap-1 text-[#050A14] font-semibold hover:underline mt-1"
                          >
                            Read full work
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <footer className="bg-[#050A14] text-white py-12 mt-20 text-center">
        <p className="text-[#FFD700] font-bold text-4xl italic mb-2">RIRI</p>
        <p className="text-gray-500 text-sm">Rwanda Innovation & Research Institute</p>
      </footer>
    </div>
  );
}