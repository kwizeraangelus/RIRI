...
export default function PendingEventsPage() {
  ...
  const fetchPendingEvents = async () => {
    ...
    try {
      const res = await fetch(getApiUrl('/api/admin/events/pending/'), {// credentials: 'include', 
  ...
  const handleApprove = async (id) => {
    if (!confirm('Approve this event?')) return;
     const token = localStorage.getItem('token');
    try {
      const res = await fetch(getApiUrl(`/api/admin/events/${id}/approve/`), {
  ...
  const handleReject = async (id) => {
    if (!feedback.trim()) return alert('Feedback required');
     const token = localStorage.getItem('token');
    try {
      const res = await fetch(getApiUrl(`/api/admin/events/${id}/reject/`), {
        method: 'POST',
       // credentials: 'include',
