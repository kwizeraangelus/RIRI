...
interface KPIs {
  ...
  pending_innovations_count?: number;
  pending_researches_count?: number;
}
...
export default function AdminDashboard() {
  ...
  const fetchApprovedBooks = async (): Promise<void> => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    const load = async () => {
      if (activeTab === 'pending') await fetchDashboard();
      else if (activeTab === 'users') await fetchUsers();
      else if (activeTab === 'events') await fetchEvents();
      else if (activeTab === 'approved') await fetchApprovedBooks();
    };
    load();
  }, [activeTab, filters]);
  ...
  const handleAction = async (id: number, action: string): Promise<void> => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/admin/upload/${id}/update`), {
        method: 'POST',
       // credentials: 'include',
        headers: { 
      ...
      if (res.ok) {
        setData(prev => ({
          ...
          kpis: { ...prev.kpis, pending_count: (prev.kpis.pending_count ?? 0) - 1 }
        }));
        setFeedbackInput('');
        setSelectedId(null);
  ...
  const handleUpdateUser = async (): Promise<void> => {
    ...
    try {
      const updateData: Partial<UserForm> = { ...userForm };
      if (!updateData.password) {
        delete updateData.password;
        delete updateData.confirm_password;
      }

      const res = await fetch(getApiUrl(`/api/admin/users/${editingUser.id}/update`), {
        method: 'PUT',
       // credentials: 'include',
        headers: { 
          'Content-Type': 'application/json',
  ...
  const handleDeleteUser = async (userId: number, username: string): Promise<void> => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/admin/users/${userId}/delete`), {
        method: 'DELETE',
       // credentials: 'include',
       headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleCreateEvent = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('/api/admin/events/create'), {
        method: 'POST',
        //credentials: 'include',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData,
  ...
  const handleUpdateEvent = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl(`/api/admin/events/${editingEvent!.id}/update`), {
        method: 'PUT',
       // credentials: 'include',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData,
  ...
  const handleDeleteEvent = async (eventId: number, eventTitle: string): Promise<void> => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/admin/events/${eventId}/delete`), {
        method: 'DELETE',
       // credentials: 'include',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const deleteBook = async (id: number, title: string): Promise<void> => {
    if (!confirm(`Are you sure you want to delete "${title}"? This action cannot be undone.`)) return;
    const token = localStorage.getItem('token');

    try {
      const res = await fetch(getApiUrl(`/api/admin/books/${id}/delete`), {
        method: 'DELETE',
       // credentials: 'include',
        headers: { 'Authorization': `Bearer ${token}` }
      });
                            ...
                            <img
                              ...
                              onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                                ...
                                if (e.currentTarget.parentElement) {
                                  e.currentTarget.parentElement.innerHTML = '<div class="text-gray-400">No Image</div>';
                                }
                              }}
                            />
                          ) : (
              ...
              <div className="mb-6 p-4 bg-white rounded-lg border">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFilterChange('university', e.target.value)}
                    className="p-2 border rounded text-sm focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                  />
                  <input
                    placeholder="Author name"
                    value={filters.author}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFilterChange('author', e.target.value)}
                    className="p-2 border rounded text-sm focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                  />
                  <input
                            ...
                            {book.cover_image_url ? (
                              <img
                                ...
                                onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                                  e.currentTarget.style.display = 'none';
                                  if (e.currentTarget.parentElement) {
                                    e.currentTarget.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400">No Cover</div>';
                                  }
                                }}
                              />
                            ) : (
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-2 text-xs text-gray-500">
                            {/* Change user.date_joined to user.created_at */}
<span>Joined: {user.created_at ? format(new Date(user.created_at), 'MMM d, yyyy') : 'N/A'}</span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEventForm({ ...eventForm, title: e.target.value })}
                          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                          required
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEventForm({ ...eventForm, location: e.target.value })}
                          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                          required
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setEventForm({ ...eventForm, icon: e.target.value })}
                          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                        >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEventForm({ ...eventForm, link: e.target.value })}
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4a772e] focus:border-transparent"
                      />
                    </div>
                  </div>
                      ...
                      <div className="flex flex-col md:flex-row">
                        ...
                        {event.photo_url && (
                          <div className="md:w-48 flex-shrink-0">
                            <img
                              src={getApiUrl(event.photo_url)}
                              alt={event.title}
                              className="w-full h-48 md:h-full object-cover"
                              onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                                e.currentTarget.style.display = 'none';
                                if (e.currentTarget.parentElement) {
                                  e.currentTarget.parentElement.innerHTML = '<div class="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400">No Photo</div>';
                                }
                              }}
                            />
                          </div>
  ...
  className="w-full py-3 px-4 text-left font-bold border rounded-lg transition bg-[#d8e5c7] text-[#4a772e] border-[#8c9c6f] hover:bg-[#c4d5b0] focus:outline-none focus:ring-2 focus:ring-[#4a772e] appearance-none cursor-pointer"
>
  <option value="">Select Action...</option>
  <option value="/admin-dashboard/create-reseach">
    Create Research
  </option>
  <option value="/admin-dashboard/create-reseach/create-expert">
    Create Expert
  </option>
</select>
