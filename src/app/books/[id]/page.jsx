...
export default function BookDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const bookId = Array.isArray(id) ? id[0] : id;
      try {
        const detailRes = await fetch(getApiUrl(`/api/university/public-detail/${bookId}/`));
        ...
        const uploaderId = detailData.user_id || detailData.userId || detailData.user?.id;
        if (uploaderId) {
          const listRes = await fetch(getApiUrl(`/api/university/public-list/?user=${uploaderId}`));
          if (listRes.ok) {
  ...
  const handleLike = async () => {
    ...
    if (liked) {
      ...
      try {
        await fetch(getApiUrl(`/api/university/${bookId}/unlike/`), {
          method: 'POST',
        });
      } catch (error) {
      ...
      try {
        await fetch(getApiUrl(`/api/university/${bookId}/like/`), {
          method: 'POST',
        });
      } catch (error) {
