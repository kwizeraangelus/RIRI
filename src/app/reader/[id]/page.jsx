...
import { ArrowLeft,  Loader2, FileText, ExternalLink, Star, Save, X } from 'lucide-react';
...
export default function ReaderPage() {
  ...
  useEffect(() => {
    const fetchDocument = async () => {
      try {
        const res = await fetch(getApiUrl(`/api/university/public-detail/${id}/`));
...
console.log("Raw path from backend:", data.file_path);

let pdfUrl = data.file_url || data.file_path;

if (!pdfUrl) throw new Error('No PDF path found');

// Build the full URL only if it's a relative path
if (!pdfUrl.startsWith('http')) {
  // Ensure the path starts with /uploads/
  const cleanPath = pdfUrl.startsWith('/') ? pdfUrl : `/${pdfUrl}`;
  pdfUrl = getApiUrl(cleanPath);
}

console.log("Final PDF URL for iframe:", pdfUrl);
  ...
  if (documentData.error) {
    return (
      <div className="min-h-screen bg-[#E0F2FE] flex items-center justify-center p-8 text-center">
  ...
  return (
    <div className="h-screen flex flex-col bg-[#E0F2FE]">
      {/* Header */}
            ...
            <div className="text-center">
              ...
              <p className="text-gray-300 mb-6">
                ...
                <span className="text-sm text-gray-400">
                  (You have read for {Math.floor(timeSpent / 60)}m {timeSpent % 60}s)
                </span>
              </p>
