...
export default function Publications() {
  ...
  useEffect(() => {
    ...
    fetch(getApiUrl("/api/researchers/1/"))
      .then((res) => res.json())
      .then(setResearcher);
  }, []);
  ...
  return (
    <div className="border p-4 max-w-md mx-auto">
      ...
      {researcher.picture && <img src={getApiUrl(researcher.picture)} alt="Bio Picture" className="w-32" />}
      <p>{researcher.bio}</p>
      ...
      {researcher.publications.map((pub, i) => (
        <div key={i} className="border-t mt-2">
          ...
          {pub.pdf && <a href={getApiUrl(pub.pdf)}>PDF</a>}
