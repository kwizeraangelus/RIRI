  ...
  const fetchUserAndEvents = async () => {
    ...
    try {
       const [userRes, eventsRes] = await Promise.all([
        ...
        fetch('http://localhost:8000/api/my-events/', { 
          headers: { 'Authorization': `Bearer ${token}` } 
        })
      ]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="bg-white rounded-2xl shadow-2xl border-2 border-blue-100 p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <input type="file" name="photo" accept="image/*" onChange={handleInputChange} className="w-full p-4 border-2 border-dashed border-blue-300 rounded-xl bg-blue-50 file:bg-blue-600 file:text-white file:py-3 file:px-8 file:rounded-lg" />
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 h-fit border">
          ...
          <div className="w-32 h-32 mx-auto mb-6 rounded-full overflow-hidden border-4 border-blue-500 shadow-xl">
            {user?.profile_image ? (
              <Image 
  src={`http://localhost:8000${user?.profile_image || user?.user?.profile_image}`} 
  alt="Profile" 
  width={128} 
  height={128} 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
            ...
            <form onSubmit={saveProfile} className="space-y-6">
              ...
              <input type="text" placeholder="campus" value={profileForm.location} onChange={e => setProfileForm(p => ({...p, location: e.target.value}))} className="w-full p-4 border border-gray-300 rounded-xl" />
